...
module.exports = {
	...
	run: async (client, message, args, users, botData) => {
		if (client.refresh.get(client.user.id)) {
			return reply(client, message, 'error', "Refresh in progress, command is impossible to use");
		}
   ...
   //if(guild.id !== "1022919634636263494" && message.author.id !== "1174345220675092482") return;
		if (!guild) return reply(client, message, 'error', 'Invalid server');

		if (removeDuplicate) {
			const msg = await message.reply("Wait, I'm bringing back members")
		...
		if (client.joins.get(id)) return reply(client, message, 'error', 'There is already an upload process in progress on this server.');
...
const webhook = new WebhookClient({ url: client.config.webhookURL2 })
		...
		const editMsg = await message.channel.send({
			content: `${client.emoji.load} **Bot Active** : \`${progress}/${users.length}\``, embeds: [
        //			content: `${client.emoji.load} **Bot Active, Starting** : \`${progress}/${client.config.ez}${users.length}\``, embeds: [

				{
					...
					description: `${client.emoji.check} Successful : \`${success}\`\n${client.emoji.error} Authorization Not Found : \`${invalid_access}\`\n${client.emoji.error} In Server : \`${already_here}\`\n${client.emoji.error} Limited Servers : \`${server_limit}\``,
					footer: {
		...
		const inter = setInterval(() => {
            editMsg.edit({
                //content: `${client.emoji.load} **Bot Active, Starting** : \`${progress}/${users.length}\``, embeds: [
        content: `${client.emoji.load} **Bot Active, Starting** : \`${progress}/${users.length}\``, embeds: [
                    {
                        title: guild.name,
                        color: client.color.default,
...
//}, 15000);
		...
		message.channel.send({
			content: `${client.emoji.check} **The bot has finished.**`, embeds: [
				{
					title: guild.name,
					...
					description: `${client.emoji.check} Successful ones : \`${success}\`\n${client.emoji.error} Those Who Cannot Find Authority : \`${invalid_access}\`\n${client.emoji.error} Already in Server : \`${already_here}\`\n${client.emoji.error} Limited Servers : \`${server_limit}\``,
				}
			]
		});
