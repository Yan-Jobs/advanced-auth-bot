...
module.exports = {
	...
	run: async (client, message, args, users, botData) => {
		...
		if (client.refresh.get(client.user.id)) {
			return reply(client, message, 'error', "Already Refreshing...");
		}
		...
		const msg = await message.channel.send({
			content: `**${client.emoji.load} Refresh in progress!** : \`${progress}/${users.length}\``, embeds: [
        //			content: `**${client.emoji.load} Refresh in progress** : \`${progress}/${client.config.ez}${users.length}\``, embeds: [
		...
		const inter = setInterval(() => {
			msg.edit({
				content: `**${client.emoji.load} Refresh in progress!** : \`${progress}/${users.length}\``, embeds: [
					{
						description: `${client.emoji.check} Successful : \`${succ}\`\n${client.emoji.error} Error : \`${error}\``,
						color: client.color.default,
						footer: {
		...
		message.channel.send({
			content: `**${client.emoji.check} Refreshed! **`, embeds: [
				{
					"description": `${client.emoji.check} Successful : \`${succ}\`\n${client.emoji.error} Error : \`${error}\``,
